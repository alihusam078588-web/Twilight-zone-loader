...
local function CollectTrainParts()
	...
	if not folder or not delivery then return false end
	...
	for _, item in ipairs(items) do
		...
		if not item.Parent then continue end
		if item.Name == "ItemCollectionHighlight" then continue end

		-- search ALL descendants for prompt (works for MeshPart nesting)
		local prompt = item:FindFirstChildWhichIsA("ProximityPrompt", true)
		if not prompt then continue end

		-- get any BasePart/MeshPart to teleport to
		local part = GetPart(item)
		local targetCFrame = part and part.CFrame or item:IsA("BasePart") and item.CFrame
		...
		local start = tick()
		while Enabled and item.Parent and prompt.Parent do
			if HRP then
				HRP.CFrame = targetCFrame * CFrame.new(0, 3, 0)
			end
			pcall(function() fireproximityprompt(prompt, 0) end)
...
local function DoCoils()
	...
	if mapFolder then
		local interact = mapFolder:FindFirstChild("Interact", true)
		if interact then
			local tesla = interact:FindFirstChild("TeslaCoil", true)
			if tesla then
				coilRoot = tesla:FindFirstChild("CoilActivators")
			end
		end
		if not coilRoot then
			coilRoot = mapFolder:FindFirstChild("CoilActivators", true)
		end
	end
	...
	for _, obj in pairs(coilRoot:GetDescendants()) do
		if not Enabled then break end
		if obj.Name ~= "Coil" then continue end
		...
		repeat
			...
			task.wait(0.05)
		until not Enabled or not prompt or not prompt.Parent or not prompt.Enabled
...
local function HandleMachine(machine)
	...
	local part = GetPart(machine)
	if not part then return end
	while vfx and #vfx:GetChildren() > 0 and Enabled do
		TPUnder(part)
		Freeze(true)
		FirePrompts(machine)
		task.wait(0.001)
...
local function run()
	...
	while Enabled do
		...
		if not didCoil then
			...
			-- ToughMachine → Golden_Machine
			local thirdChild = Interacts:GetChildren()[3]
			if thirdChild then
				local toughMachine = thirdChild:FindFirstChild("ToughMachine")
				if toughMachine then
					local goldenMachine = toughMachine:FindFirstChild("Golden_Machine")
					if goldenMachine then
						HandleMachine(goldenMachine)
					end
				end
			end
		end
...
local function DoKeplie()
	...
	local segList = segments:GetChildren()
	local segParent = segList[2]
	if not segParent then return end

	local keplie = segParent:FindFirstChild("KeplieHole")
		and segParent.KeplieHole:FindFirstChild("KeplieRoot")
		and segParent.KeplieHole.KeplieRoot:FindFirstChild("Keplie2")
	if not keplie then return end

	local prompt = keplie:FindFirstChildWhichIsA("ProximityPrompt", true)
	if not prompt then return end
	...
	while keplieEnabled do
		...
		-- teleport to keplie
		HRP.CFrame = keplie.CFrame * CFrame.new(0, 3, 0)
		task.wait(0.001)
		...
		-- if successfully fired → return to saved position
		if fired and savedCFrame then
...
MainTab:Toggle({
	Title = "Auto Keplie",
	Desc = "Not tested yet please report bugs if found",
	Value = false,
	Callback = function(state)
		keplieEnabled = state
...
local function runAutoBuy()
    local container = workspace:WaitForChild("Persistent"):WaitForChild("BaseTrain"):WaitForChild("BaseTrain"):WaitForChild("DextecItemSpawns")
    while getgenv().AutoBuyItems do
        for _, itemModel in ipairs(container:GetChildren()) do
            if itemModel and itemModel.Parent and not isBlacklisted(itemModel.Name) then
                ...
                if prompt and not buyingItems[itemModel] then
                    buyingItems[itemModel] = true
...
local AutoBuyDropdown = MainTab:Dropdown({
...
-- Auto HiFly toggle (place in MainTab)
local Players = game:GetService("Players")
local task = task
local player = Players.LocalPlayer
...
-- internal state
local autoHiFlyThread = nil
local hookedModules = {}    -- { module = { listener = conn } }
...
local function tryDisconnect(conn)
    ...
    pcall(function()
        if type(conn) == "table" then
            ...
            elseif conn.disconnect then
                conn:disconnect()
            elseif conn.disconnectEvent then
                conn:disconnectEvent()
            end
...
local function runAutoHiFly()
    while getgenv().AutoHiFly do
        task.wait(1)

        -- ensure PlayerScripts -> Client -> HiFlyController exists
        local client = player:FindFirstChild("PlayerScripts")
        if not client then continue end
        client = client:FindFirstChild("Client")
        if not client then continue end
        local hiFly = client:FindFirstChild("HiFlyController")
        if not hiFly then continue end

        -- require safely
        local success, module = pcall(require, hiFly)
        if not success or not module then continue end

        -- remove hook if module no longer exists in PlayerScripts
if hookedModules[module] and not hiFly.Parent then
    hookedModules[module] = nil
end

if hookedModules[module] then
    continue
end

        -- mark module to avoid re-hooking
        hookedModules[module] = { moduleRef = module, listener = nil }
        module._AUTO_HIFLY = true

        -- attempt to hook packets.hiFlyEvent
        local ok, packets = pcall(function()
            return require(game:GetService("ReplicatedStorage").Shared.Packets)
        end)
        if not ok or not packets or not packets.hiFlyEvent or not packets.hiFlyEvent.listen then
            -- cannot find packets; leave module marked but continue
            continue
        end

        -- attach listener; store returned connection if any
        local listenerConn = nil
        local successListen, err = pcall(function()
            listenerConn = packets.hiFlyEvent.listen(function(data)
                -- only respond while AutoHiFly is enabled
                if not getgenv().AutoHiFly then return end
                if data and data.eventType == "QTE" then
                    task.spawn(function()
                        pcall(function()
        ...
        if successListen then
            hookedModules[module].listener = listenerConn
        else
            -- if listen failed, clear hook mark so it can retry later
            hookedModules[module] = nil
            module._AUTO_HIFLY = false
        end
    end

    -- cleanup when loop exits: attempt to unhook listeners and clear marks
    for mod, info in pairs(hookedModules) do
        pcall(function()
            if info and info.listener then
                tryDisconnect(info.listener)
            end
            if mod and type(mod) == "table" then
                mod._AUTO_HIFLY = false
            end
        end)
        hookedModules[mod] = nil
    end
    autoHiFlyThread = nil
end

local function startAutoHiFly()
    if getgenv().AutoHiFly then return end
    getgenv().AutoHiFly = true
    if not autoHiFlyThread then
        autoHiFlyThread = task.spawn(runAutoHiFly)
    end
end

local function stopAutoHiFly()
    getgenv().AutoHiFly = false
    -- attempt to disconnect listeners immediately
    for mod, info in pairs(hookedModules) do
        pcall(function()
            if info and info.listener then
                tryDisconnect(info.listener)
...
local remote = game:GetService("ReplicatedStorage")
    ...
    :WaitForChild("TrainUpgradeFunction")
...
local function runAutoUpgrade()
    while autoUpgradeEnabled do
        if enemiesExist() then
            -- Pause if enemies exist
            task.wait(0.5)
        else
            -- Invoke remote if no enemies
            pcall(function()
                remote:InvokeServer({
                    UpgradeName = "StuffedRejects",
                    Action = "Contribute",
                    ContributionType = "ContributeAll"
                })
            end)
            task.wait(0.5) -- delay between calls
        end
    end
...
local function startAutoUpgrade()
    ...
    -- Check immediately
    if enemiesExist() then
        print("Enemies detected! Waiting for them to disappear...")
    else
        print("No enemies detected! Starting auto-upgrade...")
    end

    if not autoUpgradeThread then
        autoUpgradeThread = task.spawn(runAutoUpgrade)
    end
end
...
MainTab:Toggle({
    Title = "Auto Upgrade",
    Desc = "upgrading the train even when gui didn't appear ",
    Flag = "auto_upgrade_toggle",
    Value = false,
    Callback = function(state)
        if state then
            startAutoUpgrade()
...
-- Maintab toggle
MainTab:Toggle({
    Title = "Auto join exploit run",
    Desc = "Sometimes will make you join exploit run and solo runs Disable Verify Teleport if you are on Delta",
...
-- Auto Teleport to machine (safe, noclip) start/stop
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local player = Players.LocalPlayer
...
-- helper: safe teleport above a target CFrame with fallback to safe zone
local function safePlaceAbove(hrp, targetCFrame, heightOffset)
	heightOffset = heightOffset or 3
	if not hrp or not targetCFrame then return false end

	local newCFrame = targetCFrame + Vector3.new(0, heightOffset, 0)
	pcall(function() hrp.CFrame = newCFrame end)
	task.wait(0.05)

	local posY = hrp.Position.Y
	if posY and posY > 1 then return true end
...
local function isBoundingBoxValid(machine)
	...
	if cf.Position.Y < 1 then return false end
	if size.Y <= 0 or size.Y > 100 then return false end
	return true, cf, size
end

local function isRejectNearby()
	if type(isRejectNearby) == "function" then
		local ok, res = pcall(isRejectNearby)
		return ok and res
	end
	return false
end
...
local function safeFirePrompt(prompt, timeout)
	...
	while tick() - start <= timeout do
		...
		if ok then task.wait(0.06) else task.wait(0.05) end
		if not prompt.Parent then return true end
	end
	return false
end

-- handle a single machine model (works for both normal and Golden_Machine)
local function handleMachine(hrp, machine, originalCFrame)
	...
	local targetCFrame = cf - Vector3.new(0, size.Y / 2, 0)
	targetCFrame = targetCFrame + Vector3.new(0, 3, 0)

	if isRejectNearby() then task.wait(0.6) return end

	local placed = safePlaceAbove(hrp, targetCFrame, 3)
	if not placed then return end

	local prompt = machine:FindFirstChildWhichIsA("ProximityPrompt", true)

	if prompt then
		local promptTimeout = 6
		local start = tick()
		while autoTeleMach and prompt.Parent and machine:FindFirstChild("VFX") and #machine.VFX:GetChildren() > 0 do
			...
			local fired = safeFirePrompt(prompt, 0.6)
			if not fired and (tick() - start) > promptTimeout then break end
			task.wait(0.08)
		end
	else
		local start = tick()
		local timeout = 6
		while autoTeleMach and machine:FindFirstChild("VFX") and #machine.VFX:GetChildren() > 0 and (tick() - start) <= timeout do
			pcall(function() hrp.CFrame = targetCFrame end)
			task.wait(0.08)
		end
	end
...
local function startAutoTeleMach()
	...
	autoTeleMachThread = task.spawn(function()
		while autoTeleMach do
			...
			-- [Smart Mode] save original position before doing anything
			local originalCFrame = hrp.CFrame
			...
			for _, machine in ipairs(interacts:GetChildren()) do
				...
				if machine.Name == "ItemCollection" then continue end
				handleMachine(hrp, machine, originalCFrame)
				task.wait(0.05)
			end

			-- ── ToughMachine → Golden_Machine (Smart Mode) ───────────────
			if autoTeleMach then
				local toughParent = interacts:GetChildren()[3]
				if toughParent then
					local toughMachine = toughParent:FindFirstChild("ToughMachine")
					if toughMachine then
						local goldenMachine = toughMachine:FindFirstChild("Golden_Machine")
						if goldenMachine then
							handleMachine(hrp, goldenMachine, originalCFrame)
						end
					end
				end
			end

			-- [Smart Mode] return to original position after all machines done
			if autoTeleMach then
...
pcall(function()
	if root:FindFirstChild("TZ_JumpGui") then root:FindFirstChild("TZ_JumpGui"):Destroy() end
end)
...
local function showButton(state)
	if state then
		...
		TweenService:Create(btnLabel, TweenInfo.new(0.25), {TextTransparency = 0}):Play()
	else
		TweenService:Create(JumpButton, TweenInfo.new(0.2, Enum.EasingStyle.Back, Enum.EasingDirection.In), {
			Size = UDim2.new(0, 0, 0, 0),
			BackgroundTransparency = 1
		}):Play()
		TweenService:Create(btnLabel, TweenInfo.new(0.15), {TextTransparency = 1}):Play()
		task.delay(0.25, function()
			if not JumpEnabled then JumpButton.Visible = false end
		end)
	end
end
...
-- Smart Mode: re-show button if player respawns while enabled
local smartConn
...
- **Appear**: button pops in with a `Back` easing bounce from size 0 → full
- **Disappear**: shrinks back to 0 with reverse bounce
- **Press**: flashes indigo + slightly shrinks, then springs back
- Same dark card + rounded + stroke style as the black screen toggle
- Draggable on both mouse and touch
- Smart Mode: auto re-shows after respawn if toggle is still on
...
-- Plushie minigame auto-fire toggle with GUI visibility control
...
local plushGui = playerGui:WaitForChild("Main"):WaitForChild("PlushieMinigame")

local rs = ReplicatedStorage
local remote = rs:WaitForChild("Remotes"):WaitForChild("PlushieMinigame")

-- state
local autoPlushFire = false
local autoPlushThread = nil

local function startAutoPlush()
    if autoPlushFire then return end
    autoPlushFire = true

    -- hide GUI
    if plushGui then
        plushGui.Visible = false
    end

    if autoPlushThread then return end
    autoPlushThread = task.spawn(function()
        while autoPlushFire do
            pcall(function()
...
local function stopAutoPlush()
    autoPlushFire = false

    -- restore GUI visibility
    if plushGui then
        plushGui.Visible = true
    end
...
MainTab:Toggle({
    ...
    Desc = "Instantly completes the skillcheck",
    Flag = "auto_plush_perfect_toggle",
    Value = false,
    Callback = function(state)
        if state then
            startAutoPlush()
...
local function collectFromContainer(container)
    ...
    local function scan(obj)
        ...
        elseif obj:IsA("BasePart") then
            table.insert(results, {raw = obj, adornee = obj})
        else
            local basePart = obj:FindFirstChildWhichIsA("BasePart", true)
            if basePart then
                table.insert(results, {raw = obj, adornee = basePart})
            end
        end
    end
...
-- Auto use ability toggle (paste into MainTab)
local abilityModule

-- find correct ability table
for _, v in pairs(getgc(true)) do
    if type(v) == "table"
    and rawget(v, "UseAbility")
    and rawget(v, "GetCurrentAbilityData")
    and rawget(v, "Init") then
        abilityModule = v
        break
    end
end
...
local autoAbilityThread = nil

local function runAutoAbility()
    while getgenv().AutoAbility do
        task.wait(0.15)
        pcall(function()
            if not abilityModule then
                -- try to find module again if it wasn't found earlier
                for _, v in pairs(getgc(true)) do
                    if type(v) == "table"
                    and rawget(v, "UseAbility")
                    and rawget(v, "GetCurrentAbilityData")
            ...
            -- bypass cooldown and blocking states
            abilityModule.Cooldown = nil
            abilityModule.CooldownLength = nil
            abilityModule.AbilityStep = 0

            -- attempt to use ability safely
            if abilityModule and abilityModule.UseAbility then
                pcall(function()
                    abilityModule.UseAbility("AbilityBtn")
                end)
            end
        end)
    end
    autoAbilityThread = nil
end

local function startAutoAbility()
    if getgenv().AutoAbility then return end
    getgenv().AutoAbility = true
    if not autoAbilityThread then
        autoAbilityThread = task.spawn(runAutoAbility)
    end
end
...
MainTab:Toggle({
    ...
    Desc = "",
    Flag = "auto_use_ability_toggle",
    Value = getgenv().AutoAbility,
    Callback = function(state)
        if state then
            startAutoAbility()
        else
...
local AutoRejoinToggle = MainTab:Toggle({
...
local ChangelogsParagraph = ChangelogsTab:Paragraph({
    Title = "Changelogs",
    Desc = "I kinda forgot cause of my headache and my exams sorry..",
