...
local function cleanupPlayerESP(player)
	if player.Character and player.Character:FindFirstChild("TZ_PlayerESP_Gui") then
		player.Character.TZ_PlayerESP_Gui:Destroy()
	end
	if player.Character and player.Character:FindFirstChild("TZ_PlayerESP_Highlight") then
...
local function updatePlayerESPText(player)
	...
	local btext = (#backpackItems>0) and table.concat(backpackItems,", ") or "None"
	local etext = (#equipped>0) and table.concat(equipped,", ") or "None"
	label.Text = player.Name.."\nBackpack: "..btext.."\nEquipped: "..etext
end
...
local function ensureContainerESP(obj)
	...
	if obj:IsA("BasePart") then
		...
		attachmentPart = obj:FindFirstChildWhichIsA("BasePart") or obj:FindFirstChild("PrimaryPart") or obj:FindFirstChildWhichIsA("BasePart")
	end
...
local function scanContainers(forced)
...
local function onPlayerAdded(plr)
	...
	local function charAdded(char)
		...
		if hum and settings.killAlerts then
	...
	local conn1 = plr.CharacterAdded:Connect(function(c) charAdded(c) end)
	table.insert(PlayerConnections[plr],conn1)
...
local function toggleAll(state)
	settings.playersESPEnabled = state
	settings.containersESPEnabled = state
end

playersSection:CreateButton({Name = "Disable All ESP", Callback = function() toggleAll(false) end})
playersSection:CreateButton({Name = "Enable All ESP", Callback = function() toggleAll(true) end})
