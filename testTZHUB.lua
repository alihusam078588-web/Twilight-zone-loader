...
local function teleportToPart(part)
    ...
    if not hrp then return false end
    pcall(function()
        hrp.CFrame = part.CFrame + Vector3.new(0, STUFF_TELEPORT_OFFSET_Y, 0)
    end)
    return true
end
    ...
    threadRef[1] = task.spawn(function()
        ...
        while enabledRef[1] do
            ...
            if isInventoryFull() then
                ...
                notify("Auto Stuffing", "Inventory full — stopped and returned", "close")
                enabledRef[1] = false
                break
            end
            ...
            task.wait(STUFF_SCAN_DELAY)
        end

        -- cleanup: restore saved position if not inventory-full reason
        if savedPos and safeGetHRP() and not isInventoryFull() then
            pcall(function() safeGetHRP().CFrame = savedPos end)
        end
...
local function startAutoStuffing()
    if _autoStuffingEnabled then return end
    _autoStuffingEnabled = true
    runStuffingLoop({ _autoStuffingEnabled }, { _autoStuffingThread })
    notify("Auto Stuffing", "Started", "zap")
end

local function stopAutoStuffing()
    _autoStuffingEnabled = false
...
MainTab:Toggle({
    ...
    Value = false,
    Callback = function(state)
        if state then startAutoStuffing() else stopAutoStuffing() end
    end
})
        ...
        if state then
            ...
            for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
                ...
                for _, child in ipairs(enemy:GetChildren()) do
                    if child:IsA("Highlight") or child.Name == "RejectHighlight" then
                        child:Destroy()
                    end
                end

                -- Add new red highlight
            ...
            -- Toggle off: remove all highlights
            for _, enemy in ipairs(workspace.Enemies:GetChildren()) do
                for _, child in ipairs(enemy:GetChildren()) do
                    if child:IsA("Highlight") or child.Name == "RejectHighlight" then
                        child:Destroy()
