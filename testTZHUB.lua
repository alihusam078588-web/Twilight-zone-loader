...
-- Define player first so line 33 (now moved) works
local player = Players.LocalPlayer

--// Load WindUI
local WindUI = loadstring(game:HttpGet("https://github.com/Footagesus/WindUI/releases/latest/download/main.lua"))()

-- Safe access to the Machine Controller (Fixes Line 33)
...
-- =========================
-- SETTINGS & FOLDERS (Fixes WaitForChild bugs)
-- =========================
...
-- =========================
-- CORE LOGIC (Reject, Machines, Parts)
-- =========================
...
local function HandleMachine(machine)
    ...
    local cylinder = machine:FindFirstChild("Cylinder.270", true) or machine:FindFirstChildWhichIsA("BasePart", true)
    ...
    while Enabled and getProgress() < 1 do
        ...
        local targetCFrame = cylinder.CFrame * CFrame.new(0, -4, 0)

        if ok and (side == 1 or side == 2) then
            local pivot = machine:FindFirstChild("Player" .. tostring(side) .. "Pivot")
            if pivot then targetCFrame = pivot:GetPivot() * CFrame.new(0, -4, 0) end
        end
...
local function CollectTrainParts()
    ...
    for _, obj in ipairs(folder:GetChildren()) do
        if obj.Name == "Train Part" then target = obj break end
    end
