...
-- Safe helper: return a BasePart to teleport to for TrainSafeZone (or nil)
local function getTrainSafeZonePart()
    ...
    -- If TrainSafeZone is a Model or Folder, return any BasePart inside it
    return safe:FindFirstChildWhichIsA("BasePart", true)
end

-- Replace any direct TrainSafeZone teleport code with this safe wrapper:
local function teleportToSafeZone(hrp)
    ...
    local safePart = getTrainSafeZonePart()
    if safePart then
        pcall(function() hrp.CFrame = safePart.CFrame + Vector3.new(0, 5, 0) end)
    else
        dbg("teleportToSafeZone: TrainSafeZone not found")
    end
end
local function autoFarmLoop()
    ...
    local function teleportInside(inst)
    ...
    local function teleportToSafeZone()
        local safe = workspace:FindFirstChild("Persistent")
            and workspace.Persistent:FindFirstChild("Zones")
            and workspace.Persistent.Zones:FindFirstChild("TrainSafeZone")
        if safe then
            local part = safe:IsA("BasePart") and safe or safe:FindFirstChildWhichIsA("BasePart", true)
            if part then
    ...
    local function getTrainParts()
        local ic = workspace:FindFirstChild("Interacts") and workspace.Interacts:FindFirstChild("ItemCollection")
        return ic and ic:GetChildren() or {}
    end

    local function collectTrainParts()
        local collected = 0
        local parts = getTrainParts()
    ...
    while autoFarm do
        ...
        if workspace:FindFirstChild("Enemies") and workspace.Enemies:FindFirstChild("RejectKoda") then
            task.wait(1)
            continue
        end
        ...
        if #activeVFX == 0 then
            ...
            repeat
                task.wait(1)
                activeVFX = getActiveVFX()
            until not autoFarm or #activeVFX > 0
        else
...
-- AutoFarm toggle (uses the safe collectAllPickups above)
MainTab:Toggle({
    Title = "Auto Farm",
    Desc = "while using auto farm please only enable auto koda and auto train parts if you enabled other features it will get bugged",
    Value = false,
    Callback = function(state)
        autoFarm = state
        if autoFarm then
            ...
            autoFarmThread = task.spawn(function()
                ...
                local function getInteractsChildren()
                    local interacts = workspace:FindFirstChild("Interacts")
                    if not interacts then return {} end
                    return interacts:GetChildren()
                end

                local function getVFXByIndex(idx)
                    local children = getInteractsChildren()
                    local machine = children[idx]
                    if not machine then return nil end
                    return machine:FindFirstChild("VFX")
                end

                -- Teleport directly inside the VFX center
local function teleportInsideVFX(vfx)
    if not vfx then return end
    local part = vfx:IsA("BasePart") and vfx or vfx:FindFirstChildWhichIsA("BasePart", true)
    if not part then
        -- fallback: use bounding box center
        local ok, cf = pcall(function() return vfx:GetBoundingBox() end)
        if ok and cf then
    ...
    pcall(function()
        player.Character.HumanoidRootPart.CFrame = part.CFrame
                ...
                local function vfxHasChildren(vfx)
                    if not vfx then return false end
                    return (#vfx:GetChildren() > 0)
                end

                local function findFirstActiveMachineIndex()
                    local children = getInteractsChildren()
                    for i = 1, #children do
                        local m = children[i]
                        if m and m.Name ~= "ItemCollection" then
                            local vfx = m:FindFirstChild("VFX")
                            if vfx and vfxHasChildren(vfx) then
                                return i
                            end
                        end
                    end
                ...
                while autoFarm do
                    ...
                    if workspace:FindFirstChild("Enemies") and workspace.Enemies:FindFirstChild("RejectKoda") then
                        dbg("autoFarm: RejectKoda present — pausing")
                        task.wait(1.0)
                        continue
                    end

                    if isRejectNearby() then
                        dbg("autoFarm: Reject nearby — pausing")
                        task.wait(0.6)
                        continue
                    end

                    -- SAFE CALL: collect pickups only if function exists
                    if type(collectAllPickups) == "function" then
                        pcall(function() collectAllPickups(hrp) end)
                    else
                        dbg("autoFarm: collectAllPickups not available")
                    end

                    local activeIndex = findFirstActiveMachineIndex()
                    if not activeIndex then
                        local safePart = getTrainSafeZonePart()
                        if safePart then
                            pcall(function() hrp.CFrame = safePart.CFrame + Vector3.new(0, 5, 0) end)
                            dbg("autoFarm: no active machines — teleported to TrainSafeZone")
                        end
                        task.wait(0.8)
                        continue
                    end

                    local vfx = getVFXByIndex(activeIndex)
                    if not vfx then
                        task.wait(0.12)
                        continue
                    end

                    dbg("autoFarm: teleporting to VFX of machine index " .. tostring(activeIndex))

                    local attemptStart = tick()
                    local machineFinished = false
                    while autoFarm and (tick() - attemptStart) < MACHINE_ATTEMPT_TIMEOUT do
                        if workspace:FindFirstChild("Enemies") and workspace.Enemies:FindFirstChild("RejectKoda") then
                            dbg("autoFarm: RejectKoda detected while working VFX — aborting this VFX")
                            break
                        end
                        if isRejectNearby() then
                            dbg("autoFarm: Reject nearby while working VFX — aborting this VFX")
                            break
                        end

                        vfx = getVFXByIndex(activeIndex)
                        if not vfx then
                            machineFinished = true
                            break
                        end

                        teleportInsideVFX(vfx)

                        if vfxHasChildren(vfx) then
                            local parentMachine = vfx.Parent
                            if parentMachine then
                                local interaction = parentMachine:FindFirstChild("Interaction")
                                local prompt = interaction and interaction:FindFirstChildOfClass("ProximityPrompt")
                                if prompt and prompt.Parent then
                                    pcall(function() safeFirePrompt(prompt) end)
                                end
                            end
                            task.wait(MACHINE_VFX_POLL)
                        else
                            machineFinished = true
                            break
                        end
                    ...
                    if machineFinished then
                        dbg("autoFarm: machine at index " .. tostring(activeIndex) .. " finished (VFX cleared)")
                    else
                        dbg("autoFarm: timed out or aborted while waiting for VFX at index " .. tostring(activeIndex))
                    end
            ...
            autoFarm = false
            dbg("autoFarm: stopping requested")
        end
    end
})
MainTab:Toggle({
    Title = "Auto upgrade Train",
    Flag = "Auto_upgrade_Train",
    Value = false,
    Callback = function(state)
        if state then
            local Players = game:GetService("Players")
            ...
            -- Function that runs the work sequence
            local function doWork()
                local character = player.Character or player.CharacterAdded:Wait()
                local hrp = character:WaitForChild("HumanoidRootPart")
                local target = workspace.Persistent.BaseTrain.BaseTrain.TrainModel.UpgradeWall.SM_PL_Screen.SM_PL_Screen
                hrp.CFrame = target.CFrame * CFrame.new(0, -2, 0)

                local prompt = workspace.Persistent.BaseTrain.BaseTrain.TrainModel.UpgradeWall.UpgradeSurface.Attachment:FindFirstChild("ProximityPrompt")
                if prompt then
                ...
                -- Replay taps (3 clicks)
                VIM:SendMouseButtonEvent(546, 284, 0, true, game, 1)
                task.wait(0.02)
                VIM:SendMouseButtonEvent(546, 284, 0, false, game, 1)
                task.wait(0.08)
                VIM:SendMouseButtonEvent(547, 285, 0, true, game, 1)
                task.wait(0.02)
                VIM:SendMouseButtonEvent(547, 285, 0, false, game, 1)
                task.wait(0.08)
                VIM:SendMouseButtonEvent(547, 282, 0, true, game, 1)
                task.wait(0.02)
                VIM:SendMouseButtonEvent(547, 282, 0, false, game, 1)
            end
            ...
            task.spawn(function()
                while state do
                    ...
                    if not hasEnemies then
                        ...
                        -- Wait loop with enemy check
                        local waitTime = 3
                        local elapsed = 0
                        while elapsed < waitTime and state do
                            task.wait(0.5)
                            elapsed += 0.5
                            enemiesFolder = workspace:FindFirstChild("Enemies")
                            hasEnemies = enemiesFolder and #enemiesFolder:GetChildren() > 0
                            if hasEnemies then
                                -- Enemy replay script
                                VIM:SendMouseButtonEvent(586, 134, 0, true, game, 1)
                                task.wait(0.02)
                                VIM:SendMouseButtonEvent(586, 134, 0, false, game, 1)
                                task.wait(0.08)
                                VIM:SendMouseButtonEvent(582, 128, 0, true, game, 1)
                                task.wait(0.02)
                                VIM:SendMouseButtonEvent(582, 128, 0, false, game, 1)
                                task.wait(0.08)
                                VIM:SendMouseButtonEvent(582, 126, 0, true, game, 1)
                                task.wait(0.02)
                                VIM:SendMouseButtonEvent(582, 126, 0, false, game, 1)
                                break
                            end
                        end
                    else
            ...
            print("Auto Teleport & Replay disabled")
        end
    end
})
local Players = game:GetService("Players")
...
-- Coil Automation Toggle in MainTab
local RunService = game:GetService("RunService")
local Players = game:GetService("Players")
local player = Players.LocalPlayer
local character = player.Character or player.CharacterAdded:Wait()
...
-- Routine: Fire all coils with prompts
local function fireAllCoils()
    local coilActivators = workspace:FindFirstChild("Map")
        and workspace.Map:FindFirstChild("Interact")
        and workspace.Map.Interact:FindFirstChild("TeslaCoil")
        and workspace.Map.Interact.TeslaCoil:FindFirstChild("CoilActivators")

    if not coilActivators then
        -- TeslaCoil not found yet, wait and retry
        return false
    end
...
task.spawn(function()
    while true do
        if coilAutomationEnabled then
            local success = fireAllCoils()
            if success then
                teleportTo(safeZone)
            else
                -- No coils found, keep waiting and retrying
                task.wait(2)
            end
        end
        task.wait(5) -- interval between cycles
    end
end)

MainTab:Toggle({
    Title = "Auto Tesla coils",
    Desc = "Loop until coils are found, fire prompts, return to SafeZone",
    Value = false,
    Callback = function(state)
        coilAutomationEnabled = state
        WindUI:Notify({
            Title = "Coil Automation",
...
-- Button to run the remote float script safely
UniversalTab:Button({
    Title = "Run Float Script",
    Desc = "Loads float script from pastebin",
    Callback = function()
        -- defensive load: try multiple common endpoints and report status
        local url = "https://pastebin.com/raw/Y9v0TkTa"
        ...
        if ok and res then
            print("[Universal] Float script executed successfully.")
        else
            warn("[Universal] Failed to run float script.", res)
        end
    end
})
--// Feedback Tab
...
MainTab:Toggle({
    ...
    Desc = "Teleport & collect stuffing automatically",
    Value = false,
    Callback = function(state)
        AutoStuffing = state
        if state then
