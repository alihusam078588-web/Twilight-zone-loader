...
task.spawn(function()
    while true do
        if espMachines then
            for _, part in ipairs(gatherMachines()) do
                createHighlight(part.Parent, Color3.fromRGB(0,255,0))
            end
        end
...
-- Godmode (only for you)
task.spawn(function()
    while true do
        local char = LocalPlayer.Character
        if char then
            local hitFolder = char:FindFirstChild("HitPlayer", true)
            if hitFolder then
                hitFolder:Destroy()
            end
        end
...
Auto:CreateToggle({
    Name = "Auto Elevator",
    CurrentValue = false,
    Callback = function(v) autoElevator = v end
})
...
PlayerTab:CreateLabel("Godmode: ENABLED (Self Only)")
...
Credits:CreateLabel("Special Thanks: Olivia & Shelly for Rayfield idea")
