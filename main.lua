-- Twilight Zone - Rayfield-like All-in-One (single-file)
-- Created by Ali_hhjjj | Tester/Helper: GoodJOBS3
-- Special thanks: Olivia (creator of Riddance Hub) and Shelly (Riddance manager) for the Rayfield idea
...
-- GUI identity
local GUI_NAME = "TwilightZone_RayLike_AllInOne_v1"
...
startGodmodeLoop()
tzLog("Godmode loop started (HitPlayer remover).")
...
tzLog("Auto SkillCheck attachment started.")

-- ----------------- Machine gather helpers -----------------
        ...
        if espMachinesOn then
            for _, p in ipairs(gatherMachineParts()) do
                if p and p.Parent and not espMap[p] then
                    if p.Parent:IsA("Model") then createHighlightFor(p.Parent, Color3.fromRGB(0,200,0))
                    else createHighlightFor(p, Color3.fromRGB(0,200,0)) end
                end
            end
        end
...
pcall(function()
    ...
    if rem then
        ...
        if gp then
            AddStamina = gp:FindFirstChild("AddStamina")
        end
    end
end)
...
-- ----------------- Auto Teleport (to machines) with aura -----------------
-- NOTE: machine aura spam-E ONLY runs when Auto Teleport is ON.
local autoTeleportFlag = false
local machineAuraSpamE = true
task.spawn(function()
    while true do
            ...
            if #parts > 0 then
                ...
                if p then
                    ...
                    if machineAuraSpamE then
                        -- try to spam E a few times to "use" the machine
                        for i=1,6 do
                            pcall(function()
...
-- ----------------- GUI (Rayfield-like minimal implementation) -----------------
...
-- Manual teleport does NOT run the aura (only Auto Teleport does).
makeButton(pageTP, "Teleport to Random Machine", function()
...
creditLabel.Size = UDim2.new(1,0,0,36)
...
tzLog("Twilight Zone GUI loaded. Enjoy.")
