-- Twilight Zone - Rayfield-like All-in-One (fixed)
-- Author: Ali_hhjjj (kept names intact)
...
-- GUI name & cleanup
local GUI_NAME = "TwilightZone_Rayfield_v1"
...
-- returns the machines container and spirits container under workspace.Floor
local function getFloorContainers()
    local floor = Workspace:FindFirstChild("Floor")
    if not floor then return nil, nil end
    local machines = floor:FindFirstChild("Machines")
    local spirits = floor:FindFirstChild("Spirits")
    return machines, spirits
end
...
enableGodmodeLoop()
tzLog("Godmode loop started (HitPlayer removal).")
...
tzLog("Auto SkillCheck active.")

-- ---------- MACHINE GATHER (strictly workspace.Floor.Machines) ----------
local function gatherMachineParts()
    local parts = {}
    local machines, _ = getFloorContainers()
    if not machines then return parts end
    for _, child in ipairs(machines:GetChildren()) do
        if child:IsA("Model") then
            if not isFuseLike(child.Name) then
                local rep = findRepresentativePart(child)
                if rep and rep:IsA("BasePart") then
...
local function gatherSpirits()
    local _, spirits = getFloorContainers()
    local out = {}
    if not spirits then return out end
...
local espMap = {} -- map target -> highlight
...
-- ---------- Auto Teleport (to random machines) ----------
local autoTeleportFlag = false
local machineAuraSpamE = true -- aura is used only by Auto Teleport (not manual)
task.spawn(function()
    while true do
        if autoTeleportFlag then
            ...
            if #parts > 0 then
                ...
                if p then
                    teleportToPart(p)
                    -- machine aura spam E (best-effort, silent)
                ...
                -- no machines found -> go to elevator automatically
                teleportToElevator()
                task.wait(3)
            end
        end
...
-- manual teleport: does NOT spam E (aura reserved for Auto Teleport)
makeButton(pageTP, "Teleport to Random Machine", function() teleportToRandomMachine() end)
local desc = Instance.new("TextLabel", pageTP)
desc.Size = UDim2.new(1,0,0,24)
desc.BackgroundTransparency = 1
desc.Font = Enum.Font.SourceSans
desc.TextSize = 12
desc.TextColor3 = Color3.fromRGB(180,180,180)
desc.Text = "Note: Auto Teleport includes machine-aura (automatic E spam). Manual teleport does not."
...
-- Godmode and Auto SkillCheck always on; show labels
