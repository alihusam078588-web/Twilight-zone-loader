-- TZ All-in-One (fixed godmode)
-- GUI + ESP (machines/spirits) + Teleport (machine/elevator) + Auto Teleport + Auto Elevator (Message) + Godmode (personal only) + AutoSkill + Infinite Stamina
...
local GUI_NAME = "TZ_AllInOne_vFixed"
...
task.spawn(function()
    while true do
        ...
        if espSpiritsOn then
            ...
            for _, s in ipairs(containers) do
                for _, spirit in ipairs(s:GetChildren()) do
                    if spirit and not espMap[spirit] then
                        createHighlightFor(spirit, Color3.fromRGB(200,0,200))
                    end
                end
            end
        end
...
local btnTeleportNearest = addActionBtn("Teleport: Nearest Machine", teleportToNearestMachine)
local btnTeleportRandom  = addActionBtn("Teleport: Random Machine", teleportToRandomMachine)
local autoTPBtn = addToggleBtn("Auto Teleport (machines)", false, function(s) autoTeleportFlag = s end)
...
tzLog("TZ All-in-One loaded with personal godmode + infinite stamina fix.")
